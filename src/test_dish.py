...
class TestDishYearValid(SQLTestCase):
    def test_first_appeared(self):
        ...
        self.assertEqual(len(res1), 0, 'Found Dish rows with first_appeared less than 1500')
        self.assertEqual(len(res2), 0, 'Found Dish rows with first_appeared greater than 2025')
    ...
    def test_last_appeared(self):
        ...
        self.assertEqual(len(res1), 0, 'Found Dish rows with last_appeared less than 1500')
        self.assertEqual(len(res2), 0, 'Found Dish rows with last_appeared greater than 2025')

class TestDisPriceValid(SQLTestCase):
    def test_lowest_price(self):
        res1 = self.session.query(Dish).filter(Dish.lowest_price < 0).all()
        self.assertEqual(len(res1), 0, 'Found Dish rows with lowest_price less than 0')

    def test_highest_price(self):
        res1 = self.session.query(Dish).filter(Dish.highest_price < 0).all()
        self.assertEqual(len(res1), 0, 'Found Dish rows with highest_price less than 0')
